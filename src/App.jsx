...
/* ─────────────────────────────────────────────
   ...
   - Google Apps Script Web App으로 직접 전달합니다.
   ───────────────────────────────────────────── */
...
const GAS_WEBAPP_URL = "https://script.googleusercontent.com/macros/echo?user_content_key=AUkAhnTuq10lN-e65iel_OXoFVSC2vBxE10G5MLSJeDxC_CtJ2V6UWJ4WPOp87QncmHUVlGCdDxoye8PDvgzzqyERVlbik53sTZ1IpQ3Phbz2WSuvhO_4Pa5jRbtJw-hWvEqhNdW6zqxg7GfikY_Ijr2eTV8l5gwfFuTXE-GPuLNruUSBmIHa8gMz4LkRaCCAG3i-6D4mzH2vgdzHIjoaGJ3iCanY7oE2MWsaItUZ4cF5T5oJdxA3Qu6brjF0jyvm6tFDbhc-TUU7k5eNlS0xlb-uk6uthxhuQ&lib=MFgqQR43a2vg_AuS6sOZhBqDmb1FkmjRF/exec";
...
async function submitDailyReport(payload) {
  ...
  // form POST는 CORS의 영향을 받지 않아 Google Apps Script doPost 호출이 더 안정적입니다.
  const finalPayload = {
    ...payload,
    appVersion: "kbclean-final-form-post-v1",
    submitMode: "hidden-form-post-v6",
    clientSubmittedAt: new Date().toISOString(),
  };

  await submitToGasByHiddenForm(GAS_WEBAPP_URL, finalPayload);

  return {
    ok: true,
    message: "Google Drive/Sheet로 전송 요청을 보냈습니다. Google Drive를 새로고침해 확인하세요.",
  };
}

function submitToGasByHiddenForm(url, payload) {
  return new Promise((resolve, reject) => {
    try {
      const iframeName = `gas_submit_iframe_${Date.now()}`;
      ...
      // cross-origin iframe 응답은 읽지 않습니다.
      // Apps Script 처리가 끝날 시간을 고려해 4초 뒤 성공 처리합니다.
      window.setTimeout(() => {
...
function StepChecklist({ state, update, onNext }) {
  ...
  // special-order-required-v1
  // 오늘의 특별 작업지시는 모두 클릭/확인해야 다음 단계로 넘어갈 수 있습니다.
  const orderDoneCount = SITE.todayOrders.filter(
    (o) => state.checks[`order_${o.id}`]?.done
  ).length;
  const orderTotal = SITE.todayOrders.length;
  const allOrdersDone = orderTotal === 0 || orderDoneCount === orderTotal;
  const canGoNext = allOrdersDone;

  const handleNext = () => {
    if (!allOrdersDone) {
      alert(`오늘의 특별 작업지시를 모두 확인해야 다음 단계로 넘어갈 수 있습니다. (${orderDoneCount}/${orderTotal})`);
      return;
    }
    onNext();
  };
  ...
  return (
    <div>
      ...
      <div className="mt-8">
        <BigButton onClick={handleNext} disabled={!canGoNext} icon={ArrowRight}>
          {canGoNext
            ? "다음 — 필수 사진 단계로"
            : `특별 작업지시 확인 필요 (${orderDoneCount}/${orderTotal})`}
        </BigButton>
        <div
          className="text-center mt-2 text-xs font-bold"
          style={{ color: canGoNext ? KB.inkMute : KB.bad }}
        >
          {canGoNext
            ? "정기 점검 항목은 사진 촬영이 완료된 항목만 체크됩니다"
            : "오늘의 특별 작업지시를 모두 클릭해야 다음 단계로 이동할 수 있습니다."}
        </div>
      </div>
    </div>
  );
